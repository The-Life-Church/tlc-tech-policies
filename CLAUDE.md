...
- **`software/claude/templates/` is fetched at runtime** by fleet skills over raw `main` URLs (`coding:github-repo-setup` pulls `templates/gitignore`). Moving or renaming files there breaks live skills the moment the change merges — treat the paths as API.
