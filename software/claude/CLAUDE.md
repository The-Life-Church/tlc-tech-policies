...
**Claude's restrictions** (managed-settings.json) — Claude itself cannot run: `sudo`, `rm -rf`, `brew install`, any `npm install` (including bare restores), any package runner or installer from the npm ecosystem (`npx`, `npm exec`, `pnpm install/add/dlx`, `yarn add/dlx`, `bun install/add`, `bunx`), any `pip install` / `pip3 install` (including `python -m pip install`, `uv pip install`, and `uvx`), `chmod`, `chown`, `killall`, `pkill`, force push, `crontab`, `launchctl`, `systemctl`, or `curl/wget | bash`. Claude also cannot read `.env` files or secrets directly. Full list: `software/claude/managed-settings.json` in the tlc-tech-policies repo.
