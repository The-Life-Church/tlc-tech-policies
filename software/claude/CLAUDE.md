...
- **Already live, but no WORKLOG, CLAUDE.md, or GOLIVE.md** — not a new project. Ask a few quick questions (what does it do, who uses it, what needs to change?) and create the missing files from the answers. Start with the GOLIVE.md ledger since the project is deployed: the services you can identify go in as rows — `Active` only if IT provisioned them, otherwise noted for IT to confirm. Status follows the usual rule: if the deployment went through IT, the person sets it to `Live`; if it never did, keep `Status: Local` (that keeps the IT checks active) and offer to draft a systems request so IT knows the project exists and can confirm it.
...
Managed settings deliberately keep Claude from running high-risk commands — installers, `sudo`, `rm -rf`, `chmod`, force-push, `curl | bash`, process and service management (full list in `software/claude/managed-settings.json` in the tlc-tech-policies repo). Vibe-coder devices additionally carry a narrower Terminal policy of their own — it blocks `sudo`, installs, and package runners, so plenty of commands Claude can't run are still fine for the person to run themselves. These restrictions exist to prevent accidents, not to limit anyone, and **Claude never works around a block**.
