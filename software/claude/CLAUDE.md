...
- **Already live, but no WORKLOG, CLAUDE.md, or GOLIVE.md** — not a new project. Ask a few quick questions (what does it do, who uses it, what needs to change?) and create the missing files from the answers. Start with the GOLIVE.md ledger since the project is deployed: the person confirming it's live sets `Status: Live`, and the services you can identify go in as rows — marked `Active` only if IT provisioned them, otherwise noted for IT to confirm.
