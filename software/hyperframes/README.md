...
**Node ≥ 22 and FFmpeg + ffprobe are bootstrapped automatically.** The CLI's runtime is Node ≥ 22; renders shell out to system `ffmpeg` (encode) and `ffprobe` (media probing). None are bundled. Node is bootstrapped via `software/node/install.sh` when missing or too old; `software/ffmpeg/install.sh` runs **unconditionally on every run** — it's SHA-idempotent, and running it even when some `ffmpeg` is already on `PATH` means hosts with a brew or manual copy still converge to the reviewed pinned binaries at `/usr/local/bin`. So a single Mosyle script stands up the whole chain, and both stay independently deployable for hosts that want them without HyperFrames.
...
**Guarantees:**
- Idempotent — skips the CLI reinstall if the pinned version is present, and skips the skills install if they're already in the user's dir; the ffmpeg bootstrap always runs but no-ops by SHA once converged
