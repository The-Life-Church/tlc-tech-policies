...
**Node ≥ 22, FFmpeg + ffprobe, and the GitHub CLI are bootstrapped automatically.** The CLI's runtime is Node ≥ 22; renders shell out to system `ffmpeg` (encode) and `ffprobe` (media probing). None are bundled. `software/node/install.sh`, `software/ffmpeg/install.sh`, and `software/gh/install.sh` all run **unconditionally on every run** — each is idempotent at its pin (version / SHA), so hosts with brew or manual copies still converge to the reviewed pinned binaries, and Node/ffmpeg/gh pin bumps reach machines that only get this script (the standalone node and gh scripts are manual-run deploys; gh isn't a render need — it's how staff clone org repos and reach the private plugin marketplace). So a single Mosyle script stands up the whole chain, and all three stay independently deployable for hosts that want them without HyperFrames.
...
**Guarantees:**
- Idempotent — skips the CLI reinstall if the pinned version is present, and skips the skills install if they're already in the user's dir; the Node, ffmpeg, and gh bootstraps always run but no-op at their pins once converged
