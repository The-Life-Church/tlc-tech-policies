...
**Node ≥ 22 and FFmpeg + ffprobe are bootstrapped automatically.** The CLI's runtime is Node ≥ 22; renders shell out to system `ffmpeg` (encode) and `ffprobe` (media probing). None are bundled — if any is missing (or Node is too old), this script fetches and runs `software/node/install.sh` and/or `software/ffmpeg/install.sh` from the repo (the same pinned, reviewed installers) before installing the CLI, so a single Mosyle script stands up the whole chain. Both stay independently deployable for hosts that want them without HyperFrames.
...
**Guarantees:**
- Idempotent — skips the CLI reinstall if the pinned version is present, skips the ffmpeg bootstrap if `ffmpeg`/`ffprobe` are already there, and skips the skills install if they're already in the user's dir
